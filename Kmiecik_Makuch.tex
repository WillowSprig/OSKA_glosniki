	...
	\subsection{Pomiary impedancji}
		...
		\begin{table}[!ht]
			...
			\begin{tabular}{|c|c|c|c|c|}
				\hline
				\textbf{Parametr} & \makecell{\textbf{Karta katalogowa}\\ \textbf{($R=$~\SI{8}{\ohm})}} & \makecell{\textbf{Fabryczny}} & \textbf{Etap 1} & \textbf{Etap 2} \\\hline
				\hline
				$F_s$ [\si{\hertz}] & \num{47,0}  & \num{71,5} & \num{57,0} & \num{50,7}  \\\hline
				$Z_{max}$ [\si{\ohm}] & --- & \num{54,2} & \num{55,7} & \num{64,0}  \\\hline
				$R_e$ [\si{\ohm}] & \num{6,1}  & \num{10,4} & \num{10,0} & \num{10,2}  \\\hline
...
% 				$C_{as}$ [\si[per-mode=symbol]{\metre\tothe{5}\per\newton}] & \num{360e-9} & \num{165e-9} & \num{243e-9} & \num{292e-9} \\\hline
	...
	\subsection{Drgania obudowy}
		...
		Spośród zarejestrowanych jednocześnie przebiegów przemieszczeń oraz ciśnienia akustycznego wybrano te, w~których można dostrzec zależności i~przedstawiono je na rysunkach~\ref{r:wibrometr_1-4} i~\ref{r:wibrometr_5}.
		
		W~punktach \range{1}{4} (rys.~\ref{r:wibrometr_1-4}) wyraźnie widoczny jest wzrost amplitudy przemieszczeń w~paśmie \SI{260}{\hertz}; jednocześnie w~tym samym paśmie zauważalny jest spadek poziomu ciśnienia akustycznego.
		
		\begin{figure}[!ht]
			\centering
			\adjincludegraphics[width=\textwidth,trim={2cm .5cm 2cm 1cm},clip]{pkt1_4.pdf}
		...
		Na charakterystyce z~punktu~\num{5} (rys.~\ref{r:wibrometr_5}) widoczne jest lokalne maksimum amplitudy przemieszczeń dla częstotliwości~\SI{400}{\hertz}, co jednak nie znajduje odzwierciedlenia w~widmie poziomu ciśnienia akustycznego.
		
		\begin{figure}[!ht]
		\centering
		\adjincludegraphics[width=\textwidth,trim={2cm .5cm 2cm 1cm},clip]{pkt5.pdf}
