...
\section{Metodyka pomiarów}
	...
	\subsection{Metodyka podstawowa}\label{ss:metodyka}
		...
		\subsubsection{Impedancja elektryczna i parametry Thiele-Smalla}
			...
			\begin{table}[H]
				\centering
				\caption{Wyznaczenie skuteczności}
				\label{t:Skuteczność}
				\begin{tabular}{|c|c|c|}
				\hline
...
% 			Zmierzono impedancję głośnika niskotonowego w~odgrodzie, jednego głośnika w~obudowie oraz obu głośników w~obudowie.
		...
		\subsubsection{Skuteczność}
			...
			W celu weryfikacji skuteczności podawanej przez producenta wykonano pomiar poziomu dźwięku L_eq w odległości \si{2}{\m}. 
...
\section{Analiza wyników}
	...
	%Z~przyczyn technicznych parametry T-S dla tego pomiaru zostały obliczone na podstawie dostępnych materiałów (\cite{dobrucki},), a~nie wyznaczone przy pomocy systemu Pulse.
	 Parametry Thiele-Smalla zostały wyznaczone przy pomocy skryptu w~środowisku MATLAB na podstawie (\cite{dobrucki},). Następnie działanie skryptu zostało zweryfikowane w~oparciu o parametry wyznaczone w~narzędziu PULSE. 
...
% 				$C_{as}$ [\si[per-mode=symbol]{\metre\tothe{5}\per\newton}] & \num{360e-9} & \num{165e-9} & \num{243e-9} & \num{292e-9} \\\hline
		...
		\begin{figure}[H]
			...
			\caption{Impedancje głośnika w~odgrodzie oraz głośnika przy pomiarze każdą z~metod (dodanej masy i~dodanej objętości)}
			\label{r:metody}
		\end{figure}
