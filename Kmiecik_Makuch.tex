...
%	\subsection{Opis konstrukcji}
	...
	Taki układ głośników i~otworów ma wpływ na charakterystyki kierunkowości głośnika w~płaszczyźnie poziomej~\cite{kmiecik_inz}. Poglądową fotografię zestawu przedstawiono na rysunku~\ref{r:zdjecie}. %\comment{W części teoretycznej wzorek na chke kierunkowości w zależności od odległości źródeł i omówić temat!!!}
...
% 	\end{table}
	...
	\section{Metodyka pomiarów} \label{ss:metodyka}
	...
	Ze względu na zaobserwowane znaczące rozbieżności między wynikami uzyskanymi dla głośników niskotonowych, a~parametrami podanymi przez producenta w~karcie katalogowej, zdecydowano się na wykonanie dodatkowych pomiarów, uwzględniających proces eksploatacji głośnika. Oprócz badania fabrycznie nowego przetwornika wykonano pomiary na tym samym egzemplarzu poddanym procesowi zużycia poprzez dostarczenie w~dwóch etapach sygnału szumu różowego: o~mocy \SI{40}{\watt} przez czas \SI{2}{\hour} w~pierwszym etapie i~\SI{60}{\watt} przez czas \SI{10}{\min} w~drugim etapie.
...
% 		\end{picture}
	...
	Otrzymane charakterystyki kierunkowości zestawu dla głośników niskotonowych przedstawiono na rysunku~\ref{r:kierunkowosc_L}, a~dla zespołu głośnika wysokotonowego na rysunku~\ref{r:kierunkowosc_H}. 
	...
	\begin{figure}[!ht]
		...
		\begin{subfigure}[b]{.7\textwidth}
			...
			\label{r:L_pion}
		\end{subfigure}
		\caption{Charakterystyki kierunkowości głośników niskotonowych w~obudowie}
	...
	Otrzymane wyniki nie dają podstaw do jednoznacznego wskazania wpływu drgań obudowy na charakterystykę częstotliwościową zestawu. Jednocześnie dla głębszego zbadania tej zależności wskazane jest wykonanie pomiarów w~większej liczbie punktów i~przeprowadzenie dodatkowych analiz.
