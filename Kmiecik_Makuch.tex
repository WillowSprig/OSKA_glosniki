...
\section{Badany zestaw głośnikowy}
	...
	\subsection{Modyfikacje obudowy}
		...
		\begin{table}[H]
			\centering
			\caption{Opis konfiguracji obudowy i~głośników wykorzystanych podczas pomiarów \comment{Zostawiamy tak?!}}
			\label{t:obudowa}
...
\section{Modelowanie}
	...
	Dzięki stworzeniu modelu komputerowego możliwe jest łatwe i~nie wymagające wysokich kosztów testowanie różnych rozwiązań metodą elementów skończonych. Takie rozwiązanie pozwala również na dużo prostszą obserwację zjawisk akustycznych i mechanicznych zachodzących w~konstrukcji. Analiza modeli jest dużo prostsza i nie wymaga użycia zaplecza takiego jak komora bezechowa. Jednak skomplikowane modele wykorzystujące sprzężenie pomiędzy domeną mechaniczną opisującą zachowanie membrany i zawieszenia głośnika, a~domeną akustyczną opisującą propagację dźwięku w~powietrzu wymaga poznania parametrów materiałowych, z których zbudowane są wszystkie elementy. Dodatkowo problematyczne staje się stworzenie geometrii, na której możliwe stanie się wykonanie siatki elementów przy jednoczesnym odwzorowaniu szczegółów w~sposób zapewniający dobre odwzorowanie zachowania elementów modelu w połączeniu z~odpowiednio przypisanymi parametrami materiałowymi. Na taki problem natknęli się autorzy przy tworzeniu kształtu zawieszenia.  
	\comment{Opis modelu w Comsolu, co modelowaliśmy, metody (?).}
...
\section{Analiza wyników}
	...
	%Z~przyczyn technicznych parametry T-S dla tego pomiaru zostały obliczone na podstawie dostępnych materiałów (\cite{dobrucki},), a~nie wyznaczone przy pomocy systemu Pulse.
		...
		Wyraźnie widoczny jest spadek częstotliwości rezonansowej głośnika wraz z~jego zużyciem, maleje również sprawność referencyjna $\eta_0$ oraz symetria $S$, pomiar jest więc bardziej miarodajny; rośnie natomiast indeks siły $Bl$.
		...
		Porównanie obu głośników na tym samym etapie nie ukazuje wyraźnych różnic pomiędzy nimi.
